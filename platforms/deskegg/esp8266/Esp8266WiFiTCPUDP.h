...
public:
	...
	bool EnableTransparentConnections(bool transparentEnabled=true);
	void TransparentSend(char * data, int bufflength);
	int TransparentReceive(char * data, int bufflength);
    ...
    int NBRecvFrom(char * IPRemote, in_port_t &portRemote, char * data, int &dataLength, int &socket);
