...
Esp8266WiFiTCPUDP::UpdateListActiveConnections(uint8_t &clientserver, uint32_t &localPort, uint32_t &remotePort, string &remoteIP, string &conntype, uint8_t &linkID){
	...
	// resets any buffer from any spurious previous output and send
	m_UART->txBufferFlush();
...
Esp8266WiFiTCPUDP::EnableMultipleConnections(bool multipleEnabled){
	...
	dbgprintf("SEND:\r\n%s",ATCommand.c_str());
	if(!WaitFor("OK")){
		printf("Error: Missed OK reply - AT+CIPMUX\r\n");
		return;
	}
...
bool
Esp8266WiFiTCPUDP::EnableTransparentConnections(bool transparentEnabled){
	string ATCommand;

	testprintf("\r\nEntering %s ...", __PRETTY_FUNCTION__);
	// header
	ATCommand = "AT+CIPMODE=";
	...
	dbgprintf("SEND:\r\n%s",ATCommand.c_str());
	if(!WaitFor("OK")){
		printf("Error: Missed OK reply - AT+CIPMODE\r\n");
		return false;
	}
	...
	m_transparentConnMode = transparentEnabled;
	testprintf("Ended!\r\n");
	return true;
}

void
Esp8266WiFiTCPUDP::TransparentSend(char * data, int bufflength){
	string ATCommand;
...
int
Esp8266WiFiTCPUDP::TransparentReceive(char * data, int bufflength){
	int i;
	testprintf("\r\nEntering %s ...", __PRETTY_FUNCTION__);
	...
	i = 0;
	while((m_UART->readable()) && (i < bufflength)){
		data[i] = m_UART->getc();
		dbgprintf("%c",data[i]);
		i++;
	}
...
Esp8266WiFiTCPUDP::UDPSendTo(char * data, int bufflength, string IPremote, uint32_t portRemote, uint8_t linkID){
	...
	dbgprintf("\r\n");
	if(!DoubleWaitFor("SEND OK","ERROR")){
		// after 1 second timeout cleans everything
		m_UART->txBufferFlush();
		m_UART->rxBufferFlush();
		printf("Error: Missed OK reply - AT+CIPSEND\r\n");
		return false;
...
int
Esp8266WiFiTCPUDP::NBRecvFrom(char * IPRemote, in_port_t &portRemote, char * data, int &dataLength, int &socket){
	string ATCommand;
	char aValueRead;
	const char packetDelimiterPlus = '+';
	const char packetDelimiterComa = ',';
	const char packetDelimiterColon = ':';
	...
	if (m_UART->readable()){
		aValueRead = m_UART->getc();
		dbgprintf("%c",aValueRead);
		if (aValueRead != packetDelimiterPlus){
			return 0;
		}
		//packetDelimiter was found -> blocking read now
		do{
			aValueRead = m_UART->getc();
			dbgprintf("%c",aValueRead);
			if (aValueRead == packetDelimiterComa){
				break;
			}
		}while(true);
		// socket number
		if (m_multipleConnMode){
			aValueRead = m_UART->getc();
			dbgprintf("%c",aValueRead);
			socket = aValueRead - '0'; // safe to assume it's a number 0 to 4
			aValueRead = m_UART->getc();
			dbgprintf("%c",aValueRead); // the coma delimiter
		}
		if (m_showremote){
			i = 0;
			do{
				...
				bytesRd[i] = aValueRead;
				i++;
			}while(true);
			// packet length
			dataLength = atoi(bytesRd); // safe to assume it's just a number
			...
			do{
				...
				//remote IP
				IPRemote[i] = aValueRead; // outputs directly in buffer
				i++;
			}while(true);
			...
			do{
				aValueRead = m_UART->getc();
				dbgprintf("%c",aValueRead);
				if (aValueRead == packetDelimiterColon){
					portBuf[i] = '\0';
					break;
				}
				portBuf[i] = aValueRead; // outputs directly in buffer
				i++;
			}while(true);
			// remote Port
			portRemote = atoi(portBuf); // safe to assume it's just a number
		}
		else{
			i = 0;
			do{
				...
				bytesRd[i] = aValueRead; // outputs directly in buffer
				i++;
			}while(true);
			// packet length
			dataLength = atoi(bytesRd); // safe to assume it's just a number
	...
	//packetDelimiter was not found -> just spurious bytes and read was 0
	return 0;
}
